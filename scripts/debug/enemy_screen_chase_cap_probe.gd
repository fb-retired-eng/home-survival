...
func _init() -> void:
	...
	enemy.targeting_controller.receive_noise_alert(player, player.global_position)
	...
	var still_on_screen_far_offset := minf(visible_world_size.x * 0.35, enemy.targeting_controller.get_player_screen_detect_keep_radius() + 48.0)
	...
	print("enemy_screen_chase_cap_probe_screen_keep_radius=%.1f" % float(enemy.targeting_controller.get_player_screen_detect_keep_radius()))
