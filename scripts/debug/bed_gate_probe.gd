...
func _init() -> void:
	...
	await physics_frame
	await process_frame
	print("bed_gate_probe_after_sleep_state=%d" % game.game_manager.run_state)
	print("bed_gate_probe_after_sleep_phase=%s" % game.hud.wave_label.text)
	print("bed_gate_probe_after_sleep_enemy_count=%d" % _count_live_exploration_enemies(game))
	quit()
